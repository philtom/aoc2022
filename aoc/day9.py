...
def process_input(state: State, input: str) -> State:
    ...
    return reduce(process_move_increment, itertools.repeat(move.direction, move.distance), state)
...
def move_head(state: State, direction: str) -> State:
    if direction == "R":
        return dataclasses.replace(
            state, head_coord=move_right(state.head_coord)
        )
    elif direction == "L":
        return dataclasses.replace(
            state, head_coord=move_left(state.head_coord)
        )
    elif direction == "U":
        return dataclasses.replace(
            state, head_coord=move_up(state.head_coord)
        )
    elif direction == "D":
        return dataclasses.replace(
            state, head_coord=move_down(state.head_coord)
        )
    else:
...
def move_tail(state: State) -> State:
    ...
    if delta_x == 0:
        if delta_y > 1:
            return update_tail_coord(state, move_up(state.tail_coord))
        elif delta_y < -1:
            return update_tail_coord(state, move_down(state.tail_coord))
    elif delta_y == 0:
        if delta_x > 1:
            return update_tail_coord(state, move_right(state.tail_coord))
        elif delta_x < -1:
            return update_tail_coord(state, move_left(state.tail_coord))
    elif delta_x > 1:
        if delta_y > 0:
            return update_tail_coord(state, move_up(move_right(state.tail_coord)))
        elif delta_y < 0:
            return update_tail_coord(state, move_down(move_right(state.tail_coord)))
    elif delta_x < -1:
        if delta_y > 0:
            return update_tail_coord(state, move_up(move_left(state.tail_coord)))
        elif delta_y < 0:
            return update_tail_coord(state, move_down(move_left(state.tail_coord)))
    elif delta_y > 1:
        if delta_x > 0:
            return update_tail_coord(state, move_up(move_right(state.tail_coord)))
        elif delta_x < 0:
...
def update_tail_coord(state: State, tail_coord: tuple[int, int]) -> State:
    return dataclasses.replace(state, tail_coord=tail_coord, tail_history=state.tail_history | {tail_coord})
